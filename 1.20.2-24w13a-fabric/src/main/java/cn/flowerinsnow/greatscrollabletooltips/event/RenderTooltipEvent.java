...
public interface RenderTooltipEvent {
    interface Pre {
        Event<Pre> EVENT = EventFactory.createArrayBacked(Pre.class, listeners -> (screen, context, x, y, handler, focusedSlot) -> {
            for (Pre listener : listeners) {
                ActionResult actionResult = listener.preRenderTooltip(screen, context, x, y, handler, focusedSlot);
        ...
        ActionResult preRenderTooltip(HandledScreen<?> screen, DrawContext context, int x, int y, ScreenHandler handler, Slot focusedSlot);
    }
